...
def near_ten(n):
  resto = n%10
  if (resto <= 2 and resto >= 0) or resto >= 8:
    return True
  else:
    return False
...
def double_char(s):
 char = [letra+letra for letra in s]
 return ''.join(char)
...
def count_code(s):
  ...
  if len(s) < 4:
    return contador
  if len(s) == 4 and s[0:1]+s[3] == "coe":
    return contador+1
  while index < len(s)-3:
    if (
      s[index]+s[index+1] == "co" and
      s[index+3] == 'e'
      ): 
        contador +=1
    index +=1    
...
def fila_tijolos(n_peq, n_gra, meta):
  if meta - n_gra*5 <= n_peq and meta % 5 <= n_peq:
    return True
  else:
    return False
